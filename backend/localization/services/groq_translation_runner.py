...
def release_groq_lock(handler: str) -> None:
    _cache_delete(groq_lock_key(handler))
    _cache_delete(groq_lock_since_key(handler))
...
def release_stale_groq_lock(handler: str, max_starting_seconds: int = STALE_STARTING_SECONDS) -> bool:
    ...
    lock = _cache_get(groq_lock_key(handler))
    ...
    since = _cache_get(groq_lock_since_key(handler))
...
def get_groq_translation_status(handler: str) -> dict[str, Any] | None:
    payload = _cache_get(groq_result_key(handler))
...
def acquire_groq_lock(handler: str, *, source: str = 'cli') -> bool:
    ...
    if _cache_add(lock_key, source, LOCK_TTL):
        _cache_set(groq_lock_since_key(handler), time.time(), LOCK_TTL)
        return True
    return _cache_get(lock_key) == LOCK_VALUE_STARTING and source == LOCK_VALUE_RUNNING


def promote_groq_lock_to_running(handler: str) -> None:
    lock_key = groq_lock_key(handler)
    if _cache_get(lock_key) == LOCK_VALUE_STARTING:
        _cache_set(lock_key, LOCK_VALUE_RUNNING, LOCK_TTL)
        _cache_set(groq_lock_since_key(handler), time.time(), LOCK_TTL)
...
def start_groq_translation_background(handler: str) -> str:
    ...
    lock_key = groq_lock_key(handler)
    if not _cache_add(lock_key, LOCK_VALUE_STARTING, LOCK_TTL):
        return 'already_running'
    _cache_set(groq_lock_since_key(handler), time.time(), LOCK_TTL)
